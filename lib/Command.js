...
module.exports = Klass.extend({
  ...
  'getFlags': function (args) {
    ...
    if (commandIndex < 0) {
      commandIndex = args.indexOf(npath.resolve('bin/test'));
    }

    var grep = args[commandIndex + 1];
    var reporterFlag, reporterFlagIndex;

    if (!self.ignoreReporterFlag) {
  ...
  'getTestPaths': function (path, grep, paths) {
    ...
    if (!grep || self.ignoreGrep) return paths.concat(['tests/**/*.test.js']);

    path = path || npath.join(process.cwd(), 'tests');

    filesystem.readDirectory(path, function (subPath) {
      if (subPath.indexOf('.test.js') > 0 && subPath.toLowerCase().indexOf(grep.toLowerCase()) >= 0) {
        paths.push(subPath);
      }
      else if (filesystem.isDirectory(subPath)) {
  ...
  'exit': function (code) {
    ...
    var self = this;
    filesystem.recursiveDelete(npath.join(process.cwd(), 'tests/temp'));
    // self.main.kill();
    process.exit(code);
  },
});
